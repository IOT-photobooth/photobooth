...
!(function (e) {
  e.fn.customWebCam = function (i) {
    ...
    (p.settings = {}),
      (p.init = function () {
        (p.settings = e.extend(p.settings, u, i)),
          (g = p.settings.facingMode.toString().toLowerCase()),
          (h = e.extend(h, m, R())),
          p.each(async function (i, t) {
            console.log("wrapper => ", t),
              e(t).addClass("camera-capture-container"),
              (r = e(t)),
              await D(),
              console.log("$devices ==>", w),
              (n = e("<div>", { class: "video-capture-container" })),
              (A = E()),
              n.append(A),
              (S = v().hide()),
              n.append(S),
              F() && ((k = Y()), n.append(k), k.on("click", U.bind())),
              e(t).append(n),
              (o = e("<div>", { class: "canvas-image-container" })),
              (a = Z().hide()),
              o.append(a),
              e(t).append(o);
            let u = p.settings.actionButtonWrapperClass;
            u.push("camera-capture-buttons-wrapper");
            let m = e("<div>", { class: u.join(" ") });
            (c = C().hide()),
              m.append(c),
              (s = B().hide()),
              m.append(s),
              (I = b().hide()),
              m.append(I),
              w.length > 1 && !F() && ((l = G()), m.append(l), l.on("click", J.bind()), (d = f().hide()), K(), m.append(d), d.on("change", V.bind())),
              e(t).append(m),
              "" !== g && ["user", "environment"].includes(g) && (await q(), N());
          });
      });
    let E = function () {
        return e("<video>", { class: p.settings.videoClass.join(" "), width: p.settings.canvasWidth, playsinline: !0, autoplay: "autoplay" });
      },
      Z = function () {
        return e("<canvas>", { class: p.settings.canvasClass.join(" "), width: p.settings.canvasWidth });
      },
      C = function () {
        let i = p.settings.captureButtonClass;
        return i.push("image-capture-button"), e("<button>", { class: i.join(" "), text: "Capture Image" }).on("click", H.bind());
      },
      B = function () {
        let i = p.settings.retakeButtonClass;
        return i.push("image-retake-button"), e("<button>", { class: i.join(" "), text: "Retake Image" }).on("click", T.bind());
      },
      b = function () {
        let i = p.settings.okButtonClass;
        return i.push("image-ok-button"), e("<button>", { class: i.join(" "), text: "Ok Image" }).on("click", P.bind());
      },
      f = function () {
        let i = p.settings.selectClass;
        return i.push("camera-select-dropdown"), e("<select>", { class: i.join(" ") });
      },
      v = function () {
        let i = p.settings.countdownClass;
        return i.push("countdown-timer"), e("<div>", { class: i.join(" ") });
      },
      Y = function () {
        let i = p.settings.SwitchCameraIconClass;
        return i.push("switch-camera-icon"), e("<img>", { src: Q, class: i.join(" ") });
      },
      G = function () {
        let i = p.settings.showHideCameraDropdownButtonClass;
        return i.push("camera-toggle-button-icon"), e("<img>", { src: z, class: i.join(" ") });
      },
      J = function () {
        d.toggle();
      },
      ...
      D = async function () {
        try {
          return await navigator.mediaDevices.enumerateDevices().then((e) => {
            x(e);
          });
        } catch (e) {
      ...
      x = function (e) {
        console.log("mediaDevices ==>", e), (w = []);
        e.forEach((e) => {
          "videoinput" === e.kind && w.push(e);
        });
      },
      K = function () {
        d.empty(), e("<option>", { value: "", text: "Select device" }).appendTo(d);
        let i = 1;
        w.forEach((t) => {
          e("<option>", { value: t.deviceId, text: t.label || `Camera ${i++}` }).appendTo(d);
        });
      },
      q = async function () {
        let e = { audio: !1, video: { facingMode: g } };
        try {
      ...
      N = function () {
        void 0 !== t && (console.log("$currentStream ==> ", t), (window.stream = t), (A[0].srcObject = t), M(!0));
      },
      H = function () {
        S.show(),
          console.log("$video[0].videoWidth => ", A[0].videoWidth),
          console.log("$video[0].videoHeight => ", A[0].videoHeight),
          (a[0].width = A[0].videoWidth),
          (a[0].height = A[0].videoHeight);
        let i = parseInt(p.settings.captureTimeoutInSeconds);
        const t = 1e3 * i;
        let n = e("<div>", { class: "countdown-timer-number", text: i });
        S.append(n);
        let o = setInterval(function () {
          console.log("captureTimeoutInSeconds => ", i), i--, n.text(i);
        }, 1e3);
        setTimeout(function () {
          a[0].getContext("2d").drawImage(A[0], 0, 0, A[0].videoWidth, A[0].videoHeight), y(), clearInterval(o), n.remove(), S.hide();
        }, t);
      },
      U = async function () {
        console.log("Switch camera executed"), (g = "user" === g ? "environment" : "user"), await q(), N();
      },
      y = function () {
        void 0 !== t &&
      ...
      M = function (e = !0) {
        e ? (n.show(), s.hide(), a.hide(), I.hide(), c.show(), void 0 !== l && l.show()) : (n.hide(), c.hide(), void 0 !== l && l.hide(), s.show(), a.show(), I.show());
      },
      V = function () {
        let e = d.val();
        if ((console.log("$deviceId ==> ", e), "" !== e)) {
          const i = {};
          i.deviceId = { exact: e };
          const n = { video: i, audio: !1 };
      ...
      P = function () {
        ...
        const e = a[0].toDataURL(),
          i = p.settings.callbackFunction;
        console.log("typeof callBackFunction ==>", typeof window[i]), void 0 !== window[i] && "function" == typeof window[i] && window[i](e), console.log("Image => ", e), r.empty();
      },
      F = function () {
        let e = !1;
        var i;
